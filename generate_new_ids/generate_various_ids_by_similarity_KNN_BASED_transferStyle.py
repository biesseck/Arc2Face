'''
cd generate_new_ids
python generate_various_ids_by_similarity_KNN_BASED_transferStyle.py --path-dataset /nobackup3/bjgbiesseck/CASIA-Webface/imgs_crops_112x112_FACE_EMBEDDINGS_R100_WebFace42M_ArcFace --similarity-range [0.5,0.69] --num-samples-by-id 50
'''
...
if __name__ == '__main__':
    ...
    args.path_output += f'_KNN_BASED_transferStyle'
    ...
    embedds_classes_int = torch.tensor(embedds_classes_int, device='cuda:0')
    embedds_centroids   = torch.tensor(embedds_centroids, device='cuda:0')
    embedds_feats       = torch.tensor(embedds_feats, device='cuda:0')
    for idx_subj, subj_name in enumerate(all_new_embedds_labels_str):
        ...
        if idx_subj>=idxs_begin[args.part] and idx_subj<idxs_end[args.part]:
            src_real_id = all_new_embedds_labels_int[idx_subj]
            indices_original_samples_real_id = torch.where(embedds_classes_int == src_real_id)[0]
            idx_begin_src_real_id, idx_end_src_real_id = indices_original_samples_real_id[0], indices_original_samples_real_id[-1]+1
            new_id_emb = all_new_embedds[idx_subj,:].to("cuda")
            # new_id_emb = torch.unsqueeze(new_id_emb, 0)

            all_generated_images = []
            for enum_idx_sample, true_idx_sample in enumerate(range(idx_begin_src_real_id, idx_end_src_real_id)):
                new_sample_emb = new_id_emb + embedds_feats[true_idx_sample].to(torch.float16) - embedds_centroids[src_real_id].to(torch.float16)     # transfer sample perturbation
                new_sample_emb = torch.unsqueeze(new_sample_emb, 0)
                # print('new_id_emb:', new_id_emb)
                # print('new_id_emb.shape:', new_id_emb.shape)
                ...
                # Generate images:
                print(f'id {idx_subj}/{len(all_new_embedds_labels_str)} - subj \'{subj_name}\' - Generating {enum_idx_sample}/{len(indices_original_samples_real_id)} new images...')
